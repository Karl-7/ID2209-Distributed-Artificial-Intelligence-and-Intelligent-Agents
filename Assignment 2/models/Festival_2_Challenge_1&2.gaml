...
global {
	...
	list item_type<-['watch','porcelains','Antique'];//,'porcelains','my_brain'];
	list Aucters<-list_with(numberOfActneers,nil);
	int Auction_type<-1;//0:Dutch, 1: First-Price sealed-bids; 2: Vickrey
	//Centre InfoCentre;
//	Store targetstore;
	init {
...
species Person skills:[moving,fipa]{
	...
	list price<-[200,700];
 	int bid_price<-rnd(price[0],price[1]);
...
//			write("cfpMsg="+cfpMsg);
			if int(cfpMsg.contents[1])!=0 {// no result yet;
				if int(cfpMsg.contents[1])<bid_price or Auction_type!=0{//when Dutch:compute the bid; else:just send sealed bid; 
...
species Auctioneers skills:[fipa]{
	...
	action lower_price {
		...
		if (crt_price-100)>(lowest_price+1){
...
//				do end_conversation message:proposeMsg contents:['end!'];
	...
	reflex sell_it when:!empty(proposes) and auction_start=true {// and no_winner=true
		
		int bid<-0;
//		write("tick!");
		list bidders_eventually<-list_with(length(bidders),nil);
		list bids<-list_with(length(bidders),0);
		int i<-0;
		loop proposeMsg over:proposes{
			Person GUEST<-proposeMsg.sender;
			bid<-int(proposeMsg.contents[0]);
			if Auction_type=0{
...
//				do agree message:proposeMsg contents:['Agree! sell!'];
				...
				bids[i]<-bid;
				bidders_eventually[i]<-GUEST.personName;
				i<-i+1;
			}
		}
		int highest_price<-bids[0];
		int second_price<-bids[0];
		if Auction_type!=0{		
				
			winner<-bidders_eventually[0];
			loop j from: 0 to: length(bidders)-1{
				if bids[j]>highest_price{
					highest_price<-bids[j];
					winner<-bidders_eventually[j];
				}
			}
			...
			if Auction_type=1{
				...
				loop j from: 0 to: length(bidders)-1 {
					if bids[j]>second_price and winner!=bidders_eventually[j]{
						second_price<-bids[j];
					}
				}
				bid<-second_price;
			}
			if bid<lowest_price {// at lowest ptice, winner=nil
				...
				write(Actname+" winner is-----"+winner+"!!!!!!!!!!!!!!!!!!");
				
			}
//			do accept_proposal message:proposeMsg contents:['Acceptable price'];
		}
		if  Auction_type=0 or bid>=lowest_price{
			write("//////////////////////////////////////////////////////////");
			write(Actname+":winner is "+winner+" with $"+bid);	
			write("//////////////////////////////////////////////////////////");
		}
	...
	reflex actual_bidding {// and no_winner=true
		if auction_start=true and Auction_type=0{
		...
		if auction_start=true and Auction_type!=0{
			if  winner=nil{
				do start_conversation to:list(bidders) protocol:'fipa-contract-net' performative:'cfp' contents:[item,666];
			}
			else if winner!=nil{
				do start_conversation to:list(bidders) protocol:'fipa-contract-net' performative:'cfp' contents:[winner,0];
				do clean_up;
